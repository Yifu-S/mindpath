...
app.get('/api/mood/history', authenticateToken, (req, res) => {
    ...
    db.all(
        ...
        (err, rows) => {
            ...
            const decryptedMoods = rows.map(row => {
                try {
...
async function getAISupport(journalText, userId) {
    try {
        ...
        return "Thank you for sharing your thoughts. While I couldn't process this with AI right now, " +
    ...
    if (severityLevel >= 5) {
        ...
        db.run(
            `INSERT INTO crisis_logs (user_id, severity_level, detected_patterns, action_taken) 
             VALUES (?, ?, ?, ?)`,
            [userId, severityLevel, JSON.stringify(detectedPatterns), 'Alert generated']
        );
    }
...
function analyzePatterns(moodData) {
    ...
    // Find mood trend
    const recentAvg = moodData.slice(0, 7).reduce((sum, entry) => sum + entry.mood, 0) / 
                      Math.min(7, moodData.length);
    const olderAvg = moodData.slice(7, 14).reduce((sum, entry) => sum + entry.mood, 0) / 
    ...
    const commonEmotions = Object.entries(emotionCounts)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5)
        .map(([emotion, count]) => ({ emotion, frequency: count / moodData.length }));
    ...
    const recommendations = generateRecommendations(avgMood, commonEmotions, stressPatterns);
...
function generateRecommendations(avgMood, commonEmotions, stressPatterns) {
    ...
    if (avgMood < 5) {
        recommendations.push({
            ...
            description: 'Your mood has been lower than average. Consider reaching out to campus counseling.'
        });
    }
    ...
    if (hasAnxiety) {
        recommendations.push({
            ...
            description: 'Anxiety is common. Practice the 4-7-8 breathing technique daily.'
        });
    }

    const hasExamStress = stressPatterns.some(p => 
        p.context.toLowerCase().includes('exam') || p.context.toLowerCase().includes('test')
    );
    ...
    if (hasExamStress) {
        recommendations.push({
            ...
            description: 'Create a study schedule and use the Pomodoro technique for better focus.'
        });
    }
